...
- **รูปแบบปุ่มจัดการและ Pop-up (Consistent Action Buttons & Modals)**:
    - **ปุ่มลบ (Trash Button)**: ต้องมีพื้นหลังสีแดงอ่อน (`bg-red-50`), ตัวไอคอนสีแดง (`text-red-500`), ขอบมน (`rounded-xl`), ไม่มีเส้นขอบ (`border-0`), และ hover ให้สีเข้มขึ้น (`hover:bg-red-100 hover:text-red-600`)
    - **ปุ่มปิดหน้าต่าง (Close/X Button)**: ต้องไม่มีพื้นหลัง (`bg-transparent`), สีเทาอ่อน (`text-slate-400 hover:text-slate-600`), ไม่มีเส้นขอบ (`border-0`), ทรงกลม (`rounded-full hover:bg-slate-100`)
    - **Pop-up / Modal**: ต้องใช้โครงสร้างเดียวกันทุกหน้า (มี Header พร้อมปุ่มปิด มุมขวาบน, พื้นหลัง `bg-black/60 backdrop-blur-sm`, ตัวการ์ดเป็น `rounded-2xl shadow-2xl` พร้อมขอบบางๆ) เพื่อให้ทุกหน้าเหมือนกันทั้งหมด
- **ตำแหน่งปุ่มลบแถวสินค้าในหน้าฟอร์ม (Form Item Row Delete Button)**: ต้องอยู่ **ฝั่งขวาสุดของตารางเสมอ** (เช่น ในฟอร์มออกใบสั่งซื้อ PO หรือฟอร์มออกใบกำกับภาษี Invoice) เพื่อไม่ให้รบกวนแถวลำดับและช่องการเลือกสินค้า
...
## 🗄 Supabase Patterns
- โปรเจกต์นี้มีฐานข้อมูล 2 ชุด คือ **Development** (ตัวเทสใน Local) และ **Production** (ตัวใช้งานจริง)
- การดันโครงสร้าง Database (Migration) ต้องเลือกลิงก์ให้ถูกตัวก่อนเสมอ:
    - **ต้องการอัปเดต Database ตัวเทส (Local Dev)**: ให้ใช้คำสั่ง 👉 `npm run supa:link:dev`
    - **ต้องการเอาโครงสร้างขึ้นของจริง (Production)**: ให้ใช้คำสั่ง 👉 `npm run supa:link:prod`
...
## 🚀 Database Performance & Architecture (Lessons Learned)
- **หลีกเลี่ยงปัญหา N+1 Query (Bulk Operations)**: 
    - ห้ามทำการวนลูป (Loop) เพื่อเรียกคำสั่ง Update หรือ Insert ลง Database ทีละรายการเด็ดขาด เนื่องจากจะทำให้เกิดคอขวดเมื่อมีจำนวนข้อมูลเยอะขึ้น
    - ให้รวบรวมข้อมูลทั้งหมดใส่ Array แล้วใช้คำสั่ง `.insert([...])` เพียงครั้งเดียว หรือหากเป็นการ Update ให้สร้างฟังก์ชัน RPC บน Supabase เพื่อรองรับการอัปเดตแบบ Batch
- **การจัดการ Race Conditions ในการคำนวณ (Atomic Updates)**: 
    - ห้ามใช้กระบวนการ "ดึงข้อมูล -> คำนวณฝั่ง JS -> สั่งอัปเดต" (Read-then-Write) โดยเฉพาะระบบสต็อกหรือยอดเงินที่อาจมีการกดพร้อมกันจากหลายผู้ใช้งาน
    - ต้องสร้างฟังก์ชัน RPC บนฐานข้อมูล (เช่น `UPDATE table SET qty = qty + delta`) เพื่อให้ฐานข้อมูลเป็นผู้จัดการเรื่อง Concurrency อย่างปลอดภัย
- **การจัดการ Rollback ใน Multi-step Operations (Compensation Logic)**: 
    - เมื่อต้องทำคำสั่งหลายขั้นตอนต่อเนื่องกัน (เช่น คืนสต็อกเก่า แล้ว ตัดสต็อกใหม่) หากไม่มีระบบ Transaction ให้ระมัดระวังกรณีที่สเต็ปหลังล้มเหลว
    - ต้องสร้างบล็อก `try-catch` คอยจับ Error แล้วมีคำสั่ง "ย้อนกลับ (Revert)" สเต็ปก่อนหน้าเสมอเพื่อป้องกันข้อมูลไม่สอดคล้องกัน
- **การค้นหาหรืออ้างอิงข้อมูลอย่างรัดกุม (Robust Queries)**: 
    - ห้ามใช้คำสั่ง `ilike` กับฟิลด์ข้อความ (เช่น `remark` หรือ `notes`) เพื่อใช้ในการอ้างอิง หรือ ค้นหา Log ที่ต้องการทำลาย/แก้ไข
    - ต้องใช้ฟิลด์ที่เป็น Foreign Key หรือ Identifier ที่ชัดเจนเสมอ (เช่น `source_type` และ `source_id`) เพื่อความแม่นยำและป้องกันความผิดพลาดหากข้อความในระบบถูกเปลี่ยนแปลง
