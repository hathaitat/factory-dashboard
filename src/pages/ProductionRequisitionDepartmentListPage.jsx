...
const ProductionRequisitionDepartmentListPage = () => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const [linesData, requisitions] = await Promise.all([
                productionService.getLines(),
                productionMaterialService.getRequisitions({ dateFrom, dateTo })
            ]);
            ...
            if (Array.isArray(requisitions)) {
                requisitions.forEach(req => {
                    ...
                    if (!summaryMap[lineId]) {
                        summaryMap[lineId] = { req_count: 0, total_items: 0 };
                    }
                    summaryMap[lineId].req_count++;
                    summaryMap[lineId].total_items += (req.items?.length || 0);
                });
            }
            setMonthlySummary(summaryMap);
        } catch (error) {
            console.error('Error loading requisitions summary:', error);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="p-6">
            ...
            <PageHeader
                ...
                subtitle="เลือกแผนกเพื่อดูรายการเบิกวัตถุดิบเข้าสู่แผนกการผลิต"
            />
                ...
                {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {lines.map((line, index) => {
                            const summary = monthlySummary[line.id] || { req_count: 0, total_items: 0 };

                            return (
                                <div key={line.id} className={`rounded-xl p-5 border-2 shadow-md flex flex-col justify-between transition-all cursor-pointer group hover:shadow-lg ${getDeptColorClass(index)}`} onClick={() => navigate(`/dashboard/production/requisitions/line/${line.id}`)}>
                                    <div>
                                        ...
                                        <div className="bg-white/60 rounded-lg p-3 mb-4 space-y-2 border border-white/50 shadow-sm">
                                            <div className="text-xs font-semibold text-textMuted mb-1 flex items-center gap-1">
                                                <Calendar size={12} /> ข้อมูลใบเบิกเดือนนี้ ({new Date().toLocaleDateString('th-TH', { month: 'short', year: '2-digit' })})
                                            </div>
                                            <div className="flex justify-between items-center text-sm">
                                                <span className="text-textMuted">จำนวนเอกสารใบเบิก:</span>
                                                <span className="font-medium">{summary.req_count.toLocaleString()} ใบ</span>
                                            </div>
                                            <div className="flex justify-between items-center text-sm">
                                    ...
                                    <button className="btn btn-primary w-full shadow-md group-hover:shadow-lg transition-all flex items-center justify-center gap-2 mt-4">
                                        <FileText size={18} /> ดูประวัติการเบิก
                                    </button>
                                </div>
                            );
                        })}
...
export default ProductionRequisitionDepartmentListPage;
