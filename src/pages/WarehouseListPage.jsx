...
import { Building2, Search, Package, Settings, Edit2, Trash2, Plus, Bot, MapPin, Phone, User, Save, X } from 'lucide-react';
...
const WarehouseListPage = () => {
    ...
    const runTestBot = async () => {
        ...
        try {
            ...
            if (!suppliers || suppliers.length === 0) {
                alert('ต้องมีผู้ขาย (Supplier) อย่างน้อย 1 รายในระบบเพื่อสร้างใบสั่งซื้อ');
                return;
            }
            const defaultSupplier = suppliers[0];

            const mainWh = warehouses.find(w => w.is_default);
            const suppWh = warehouses.find(w => w.type === 'supplier');
            const customWh = warehouses.find(w => w.type === 'custom' && !w.is_default);

            const testWarehouses = [
                { wh: mainWh, name: 'คลังหลัก' },
                { wh: suppWh, name: 'คลังผู้ขาย' },
                { wh: customWh, name: 'คลังสร้างเอง' }
            ].filter(t => t.wh);

            if (testWarehouses.length === 0) {
                alert('ไม่พบคลังสินค้าสำหรับทดสอบ กรุณาสร้างคลังก่อน');
                return;
            }

            for (const { wh, name } of testWarehouses) {
                const poData = {
                    supplier_id: defaultSupplier.id,
                    delivery_warehouse_id: wh.id,
                    ...
                    grand_total: 1070,
                    items: [
                        { description: `Test Mat ${name} (${Date.now().toString().slice(-4)})`, quantity: 50, unit: 'PCS', unit_price: 20, total: 1000 }
                    ]
                };

                const createdPo = await supplierPoService.createSupplierPo(poData);
                await supplierPoService.updateStatus(createdPo.id, 'completed');
            }

            alert(`Test Bot ทำงานสำเร็จ! สร้าง PO และนำของเข้าคลัง ${testWarehouses.length} คลังเรียบร้อยแล้ว`);
            if (activeWarehouseId) {
                loadInventory(activeWarehouseId);
            }
        } catch (error) {
            console.error('Test Bot Error:', error);
            alert('เกิดข้อผิดพลาดในการรัน Test Bot');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    // Filter inventory by tab and search
    const filteredInventory = inventory.filter(i => {
        const matchTab = i.product_type === activeTab;
        ...
        <div style={{ padding: '0 1rem 2rem 1rem' }}>
            ...
            >
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    >
                        <Bot size={20} /> Test Bot: จำลองรับของเข้าคลัง
                    </button>
                    <button
