DROP FUNCTION IF EXISTS get_production_summary_by_date(DATE, DATE, UUID);
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE((
            ...
            WHERE l.plan_id IN (
                ...
                WHERE p2.product_name = p.product_name AND p2.line_id = p.line_id
    ...
    LEFT JOIN public.production_lines pl ON p.line_id = pl.id
    WHERE p.plan_date >= p_date_from AND p.plan_date <= p_date_to
      AND (p_line_id IS NULL OR p.line_id = p_line_id)
    GROUP BY p.line_id, pl.name, p.product_name, p.product_code;
END;
