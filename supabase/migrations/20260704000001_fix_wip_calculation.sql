-- Fix WIP Calculation to include produced quantity
-- WIP = Requisitioned - Returned - Produced
...
BEGIN
    ...
    prod_summary AS (
        SELECT 
            plan_id AS target_plan_id,
            SUM(quantity_produced + COALESCE(quantity_defect, 0)) AS total_prod
        FROM public.production_daily_logs
        WHERE line_id = p_line_id AND log_date = p_date
        GROUP BY plan_id
    )
    SELECT 
